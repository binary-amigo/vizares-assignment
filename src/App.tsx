...
import { Plus, Search, Edit2, Trash2, CheckCircle, X, GripVertical } from 'lucide-react';
...
function App() {
  ...
  const handleDragEnd = (result: any) => {
    ...
    const updatedTasks = tasks.map(task => {
      const filteredTask = items.find(item => item.id === task.id);
    ...
    <h1 className='text-4xl text-center p-4 font-extrabold text-blue-500'>Manage tasks easily</h1>
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between items-start mb-2">
                            <h3 className={`text-lg font-semibold ${task.completed ? 'text-green-700' : 'text-gray-800'}`}>
                              {task.title}
                            </h3>
      ...
      {filteredTasks.length === 0 && (
        <div className="text-center text-gray-500 mt-12">
          {searchTerm ? 'No tasks found' : 'No tasks yet. Click the + button to add one!'}
        </div>
      )}
    </div>
  );
